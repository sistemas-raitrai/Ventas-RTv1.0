...
window.diagnosticarRecuperacionMasivaNominaFinal =
async function ({
  soloErrorContactoUndefined = true
} = {}) {
  console.log(
    "======================================================"
  );
  ...
  console.log(
    "🩺 DIAGNÓSTICO MASIVO RECUPERACIÓN NÓMINA FINAL"
  );
  ...
  try {
    ...
    console.error(
      "❌ No se pudieron leer las inscripciones pendientes.",
      error
    );
  ...
  console.log(
    `📦 Errores encontrados: ${snap.size}`
  );

  // ================================================================
  // 2. CONSTRUIR CANDIDATOS NOMINA FINAL
  // ================================================================
  ...
  for (
    ...
    /*
      Para este barrido nos interesan los formularios
      de Nómina Final que actualizan pasajeros ya
      existentes de Sistema de Pagos.
    */
    if (!actualizaExistente) {
      continue;
    }
  ...
  console.log(
    `🎯 Candidatos Nómina Final: ${candidatos.length}`
  );
  ...
  candidatos.forEach(
    (item) => {
      const key =
        `${item.idGrupo}__${item.inscripcionId}`;
  ...
  for (
    ...
    let fichaOficialCompleta =
      false;

    let fechaOficial =
      null;

    let formularioPendienteMasNuevo =
      false;

    let payloadTieneSalud =
      tieneDatosSaludPayload(
        item.payload
      );
    ...
    if (!item.idGrupo) {
      ...
      detalle =
        "El pendiente no identifica un grupo.";
      ...
      resultados.push({
        ...
        detalle,
        existeOficial,
        fichaOficialCompleta,
        formularioPendienteMasNuevo,
        payloadTieneSalud
      });
    ...
    if (!item.inscripcionId) {
      clasificacion =
        "REVISAR_SIN_INSCRIPCION_ID";

      detalle =
        "El pendiente no trae inscripcionSistemaPagosDocId ni documentoNormalizado.";
    ...
    if (!grupoSnap.exists()) {
      clasificacion =
        "REVISAR_GRUPO_NO_EXISTE";

      detalle =
        "No existe el documento del grupo en ventas_cotizaciones.";
    ...
    if (
      ...
      /*
        Si hay varios pendientes de la misma persona,
        identificamos cuál es el más reciente.
      */
      ...
      if (
        ...
        clasificacion =
          "OMITIR_DUPLICADO_ANTIGUO";

        detalle =
          `Existe otro formulario pendiente más reciente: ${masReciente.pendienteId}.`;
    ...
    // --------------------------------------------------------------
    // NO EXISTE PASAJERO OFICIAL
    // --------------------------------------------------------------

    if (!existeOficial) {
      clasificacion =
        "REVISAR_SIN_INSCRIPCION_OFICIAL";

      detalle =
        "El formulario dice que actualiza una inscripción existente, pero el pasajero oficial no fue encontrado.";
    ...
    const fechaPendienteMs =
      fechaAValor(
        item.fechaPendiente
      );

    const fechaOficialMs =
      fechaAValor(
        fechaOficial
      );

    /*
      Solo consideramos el oficial "posterior"
      cuando tiene una fecha válida y estrictamente
      posterior al formulario fallido.
    */
    const oficialPosterior =
      fechaOficialMs > 0 &&
      fechaPendienteMs > 0 &&
      fechaOficialMs >
        fechaPendienteMs;
    ...
    // --------------------------------------------------------------
    // YA EXISTE UNA FICHA COMPLETA MÁS NUEVA
    // --------------------------------------------------------------
    ...
    if (
      ...
      clasificacion =
        "OMITIR_FICHA_COMPLETA_POSTERIOR";

      detalle =
        "La inscripción oficial ya tiene ficha médica completa y su fecha es posterior al formulario fallido.";
    ...
    // --------------------------------------------------------------
    // FICHA COMPLETA PERO NO PODEMOS DEMOSTRAR QUE SEA MÁS NUEVA
    // --------------------------------------------------------------
    ...
    if (
      ...
      clasificacion =
        "REVISAR_FICHA_YA_COMPLETA";

      detalle =
        "La inscripción oficial figura con ficha médica completa, pero no se puede asegurar que sea posterior al formulario fallido.";
    ...
    if (!payloadTieneSalud) {
      clasificacion =
        "REVISAR_PAYLOAD_SIN_SALUD";

      detalle =
        "El pendiente parece Nómina Final, pero el payload no contiene el mapa salud.";
    ...
    detalle =
      "Inscripción oficial existente de Sistema de Pagos con ficha pendiente y formulario fallido recuperable.";
  ...
  const resumen = {
    erroresTotalesFirestore:
      snap.size,

    candidatosNominaFinal:
      candidatos.length,
    ...
    omitirDuplicadoAntiguo:
      contar(
        "OMITIR_DUPLICADO_ANTIGUO"
      ),

    omitirFichaCompletaPosterior:
      contar(
        "OMITIR_FICHA_COMPLETA_POSTERIOR"
      ),

    revisarFichaYaCompleta:
      contar(
        "REVISAR_FICHA_YA_COMPLETA"
      ),

    revisarSinInscripcionOficial:
      contar(
        "REVISAR_SIN_INSCRIPCION_OFICIAL"
      ),
    ...
    revisarGrupoNoExiste:
      contar(
        "REVISAR_GRUPO_NO_EXISTE"
      ),

    revisarSinInscripcionId:
      contar(
        "REVISAR_SIN_INSCRIPCION_ID"
      ),

    revisarPayloadSinSalud:
      contar(
        "REVISAR_PAYLOAD_SIN_SALUD"
      )
  };
  ...
  // ================================================================
  // 7. DEJAR RESULTADO DISPONIBLE
  // ================================================================

  window.__diagnosticoRecuperacionNominaFinal =
    {
      resumen,
      resultados
    };

  window.__candidatosRecuperacionNominaFinal =
    resultados.filter(
      (item) =>
        item.clasificacion ===
        "RECUPERAR"
...
window.recuperarInscripcionPublicaFallida = async function ({
  ...
  console.log(
    "🔧 RECUPERACIÓN INSCRIPCIÓN PÚBLICA FALLIDA"
  );
  ...
  try {
    ...
    if (!pendienteSnap.exists()) {
      console.error(
        "❌ No existe el documento pendiente.",
        {
          pendienteId: id
        }
      );
    ...
    if (
      estadoOriginal !== "error"
    ) {
      console.error(
        "❌ El documento indicado no está en estado error.",
        {
          pendienteId: id,
          estado:
            pendiente.estado || ""
      ...
      return {
        ok: false,
        motivo: "pendiente_no_esta_en_error",
        pendienteId: id,
        estado:
          pendiente.estado || ""
      };
    ...
    if (!payloadOriginal) {
      console.error(
        "❌ El documento no contiene payload."
      );

      return {
        ok: false,
        motivo: "payload_no_disponible",
        pendienteId: id
      };
    }

    // ================================================================
    // 2. IDENTIFICAR GRUPO, FASE, TOKEN Y PASAJERO
    // ================================================================

    const idGrupoPendiente =
      cleanText(
        pendiente.idGrupo ||
        payloadOriginal?.grupo?.idGrupo ||
        ""
      );
    ...
    const fase =
      cleanText(
        ...
        payloadOriginal.tipoInscripcion ||
        ""
      );
    ...
    /*
      Algunos documentos antiguos guardaron el token
      solamente dentro del payload.
    */
    const token =
      cleanText(
        pendiente.token ||
    ...
    /*
      Para Nómina Final proveniente de Sistema de Pagos,
      este es el ID oficial que debe actualizarse.
    */
    const inscripcionId =
      cleanText(
        payloadOriginal
    ...
    const actualizaExistente =
      payloadOriginal
        .actualizaInscripcionExistente === true;
    ...
    if (
      ...
      console.error(
        "❌ Faltan datos requeridos para recuperar.",
        {
          pendienteId: id,
          idGrupo:
            idGrupoPendiente,
      ...
      return {
        ok: false,
        motivo:
          "datos_recuperacion_incompletos",
    ...
    // ================================================================
    // 3. VERIFICAR QUE CORRESPONDA AL GRUPO ABIERTO
    // ================================================================
    ...
    if (
      ...
      console.error(
        "❌ El documento pertenece a otro grupo.",
        {
          pendienteId: id,
    ...
    /*
      Excepción fundamental:

      nomina_final + actualizaInscripcionExistente=true
      DEBE poder actualizar un pasajero activo de
      Sistema de Pagos.

      Para las demás fases se mantiene el bloqueo
      contra duplicados.
    */
    const esActualizacionNominaFinal =
      esNominaFinal &&
      actualizaExistente === true;
    ...
    if (!puedeReprocesar) {
      console.warn(
        "⚠️ La inscripción ya existe activa y esta fase no permite actualizarla.",
        {
          idGrupo:
            idGrupoPendiente,
    ...
    // ================================================================
    // 5. CORREGIR COMPATIBILIDAD DEL PAYLOAD ANTIGUO
    // ================================================================
    ...
    /*
      El formulario antiguo almacenó "nombre",
      pero el backend esperaba "nombreCompleto".
      Guardamos ambas propiedades.
    */
    const contactoPrincipalCorregido = {
      ...
      nombre:
        cleanText(
          contactoPrincipalOriginal.nombre ||
          contactoPrincipalOriginal.nombreCompleto ||
          ""
        ),
    ...
    const contactoSecundarioCorregido = {
      ...
      nombre:
        cleanText(
          contactoSecundarioOriginal.nombre ||
          contactoSecundarioOriginal.nombreCompleto ||
          ""
        ),
    ...
    if (confirmar) {
      const accion =
        ...
        existeInscripcion
          ? (
              "Se actualizará la inscripción existente de " +
              "Sistema de Pagos con la ficha médica recuperada."
            )
          : (
              "Se reenviará el formulario para crear o " +
              "recuperar la inscripción correspondiente."
            );

      const ok =
        window.confirm(
          `RECUPERAR FICHA FALLIDA\n\n` +
          `Grupo: ${idGrupoPendiente}\n` +
          `Pasajero: ${nombre || "(sin nombre)"}\n` +
          `Documento: ${documento || inscripcionId}\n` +
          `Fase: ${fase}\n\n` +
          `${accion}\n\n` +
          `Se corregirá la compatibilidad del contacto principal:\n` +
          `${contactoPrincipalCorregido.nombreCompleto || "(sin nombre)"}\n\n` +
          `El documento original no será eliminado.\n\n` +
          `¿Continuar?`
        );
    ...
    // ================================================================
    // 7. CREAR NUEVO PENDIENTE CORREGIDO
    // ================================================================
    ...
    const nuevoRef =
      await addDoc(
        ...
        ),
        {
          ...
          origen:
            "recuperacion_manual_corregida_grupo_js",
    ...
    await createHistoryEntry({
      ...
      titulo:
        "Ficha pública reenviada para recuperación",

      mensaje:
        `${getDisplayName(state.effectiveUser)} reenvió para recuperación la ficha de ${nombre || documento || inscripcionId}. ` +
        (
          esActualizacionNominaFinal
            ? "La ficha será aplicada sobre la inscripción existente de Sistema de Pagos."
            : "La inscripción será procesada nuevamente."
        ),
    ...
    console.log(
      "✅ Nuevo documento pendiente corregido creado.",
      {
        pendienteOriginalId:
          id,
    ...
    // ================================================================
    // 9. ESPERAR RESULTADO DE LA CLOUD FUNCTION
    // ================================================================
    ...
    for (
      ...
      if (
        ...
        console.error(
          "❌ La recuperación volvió a fallar.",
          {
            nuevoPendienteId:
              nuevoRef.id,
    ...
    // ================================================================
    // 10. RECARGAR NÓMINA Y SU RESUMEN
    // ================================================================

    /*
      El trigger que actualiza nomina_resumen necesita
      un margen después de modificar la inscripción oficial.
    */
    await esperar(
      1800
    );
    ...
    console.log(
      "✅ RECUPERACIÓN TERMINADA",
      {
        ...
        fichaMedicaCompletaEsperada:
          payloadCorregido
            .fichaMedicaCompleta === true,

        nominaFinalCompletaEsperada:
          payloadCorregido
            .nominaFinalCompleta === true
      }
    );
