...
function renderLayoutTop(user) {
  return `
    ...
    <header class="ventas-header app-header">
      ...
      <nav class="ventas-menu app-nav" aria-label="Navegación principal">
        <div id="layout-menu-links" class="ventas-menu-inner menu-pills">
          ${renderMenuLinks(user)}
        </div>
      </nav>
    </header>
...
function updateLayoutUser(user) {
  ...
  if (usuarioConectado) {
    usuarioConectado.textContent = user?.email || "";
  }
  ...
  // Aviso global para que cada página pueda volver a enlazar su lógica
  // de "navegar como" después de que Firebase entregue el usuario.
  window.dispatchEvent(new CustomEvent("ventas-layout-ready", {
    detail: { user }
  }));
}

function bindStaticLayoutEvents() {
  ...
  if (btnLogout && !btnLogout.dataset.bound) {
    ...
    btnLogout.addEventListener("click", async (e) => {
      ...
      try {
        // Limpia el posible "navegar como"
...
function mountLayoutTop() {
  ...
  // Renderiza UNA sola vez el layout completo
  slot.innerHTML = renderLayoutTop(null);

  // Enlaza solo eventos realmente estáticos del header
  bindStaticLayoutEvents();

  // Avisa que el layout ya existe en DOM
  window.dispatchEvent(new CustomEvent("ventas-layout-ready", {
    detail: { user: null }
  }));

  // Cuando Firebase responda, SOLO actualizamos texto / menú,
  // pero NO reescribimos todo el header.
